...
public class DoublyLinkedList <T>{
    ...
    public void addFirst(T item){                                 //adds a node to beginning
        if(isEmpty()){  
    ...
    @SuppressWarnings("unchecked")
    public void addLast(T item){                                 // adds a node to the last
        if(isEmpty()){
            head = new Node(item, null, null);
            tail=head;
        }else{
    ...
    public T peekLast(){                                      //returns the last data in the doubly linked list
        if(isEmpty()) throw new RuntimeException("Empty List");
        return tail.data;
    }
